...
export default function EditInfoUser() {
  ...
  const saveClicked = (event) => {
    ...
    if (ValidateInfo(state, setState)) {
      ...
      const person = {
        ...
        phone: phone,
        birthDate: birthDate,
        packageName: state.userBooking.packageName,
        notes: notes,
  ...
  const [price, setPrice] = React.useState(state.userBooking.OTCCharges ?? 0);
  ...
  useEffect(() => {
    ...
    setState((state) => ({ ...state, service: state.userBooking.packageName }));
  ...
  useEffect(() => {
    setChanged(
      firstname !== state.userBooking.fullname ||
        ...
        service !== state.userBooking.packageName ||
  ...
  const [birthDate, handleBirthDateChange] = React.useState(
    state.userBooking.birthDate ?? null
  );
          ...
          >
            ...
            {/* <Grid item xs={12} md={12}>
              <FormControl className={classes.formControl} fullWidth required>
                ...
                >
                  {Packages.map((item) => (
                    <MenuItem value={`${item.packageName} (male)`} onClick={() => setPrice(parseInt(item.malePrice.substr(1)))}>
                      {`${item.packageName} (male) - ${item.malePrice}`}
                    </MenuItem>
                  ))}

                  {Packages.map((item) => (
                    <MenuItem value={`${item.packageName} (female)`}  onClick={() => setPrice(parseInt(item.femalePrice.substr(1)))}>
                      {`${item.packageName} (female) - ${item.femalePrice}`}
                    </MenuItem>
                  ))}
                </Select>
              </FormControl>
