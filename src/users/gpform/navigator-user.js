...
import GPBookService from './../../services/GPBookService';
...
function timePassed (bookingDate)
{
    ...
    bookingDate = new Date((new Date(bookingDate)).getTime() + 60*60*24*1000)
...
export default function NavigatorUser(props) {
    ...
    useEffect(() => {
        document.title = "Private GP Registration Form"
     }, []);
...
if (props.pathId && props.pathId.length === 24)
        {
            GPBookService.getBookingById(props.pathId).then( res => {
