...
import PaediatricianBookService from "./../../services/PaediatricianBookService";
...
export default function NavigatorUser(props) {
    ...
    useEffect(() => {
        document.title = "Paediatrician GP Registration Form";
     }, []);
...
if (props.pathId && props.pathId.length === 24)
        {
            PaediatricianBookService
              .getBookingById(props.pathId)
              .then((res) => {
                ...
                if (result) {
                  if (
                    ...
                    (result.status !== "booked" ||
                      timePassed(result.bookingDate))
                  ) {
                    setState((state) => ({
                      ...state,
                      pathIdNotFound: false,
                      timePassed: true,
                    }));
                    setLoaded(true);
                  } else if (
